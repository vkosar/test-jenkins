        ...
        stage('Deploy') {
            ...
            if (isDeploy) {
                ...
                sshagent (credentials: ['deploy-dev']) {
                    ...
                    sh 'ssh -o StrictHostKeyChecking=no user@localhost bash --login -c env'
                    sh 'ssh -o StrictHostKeyChecking=no user@localhost bash --login -c \'forever restart --workingDir test-jenkins test-jenkins/app.js\''
                }
